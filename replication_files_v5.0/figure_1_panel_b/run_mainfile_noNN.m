...
nd        = 1e2;             % number of orthogonal-reduced-form (B,Sigma,Q) draws
iter_show = 10;            % display iteration every iter_show draws
...
y_new = storevefh(nonzero_idx, :);
x_new = structuraldraws(nonzero_idx, :);

y_pred = predict(net, x_new);

figure;
plot(1:size(y_new,1), y_new, 'r', 'LineWidth', 1.5); % Red solid line
hold on;
plot(1:size(y_pred,1), y_pred, 'bo', 'LineWidth', 1.5); % Blue circles
hold off;

legend('Actual (y_{new})', 'Predicted (y_{pred})');
xlabel('Observation');
ylabel('Value');
title('Neural Network Forecast vs Actual');
grid on;
